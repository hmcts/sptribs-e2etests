  ...
  async checkPageLoads(page: Page, accessibilityTest: Boolean): Promise<void> {
    await Promise.all([
      ...
      ...Array.from({ length: 3 }, (_, index) => {
        const textOnPage = (confirm_content as any)[`textOnPage${index + 1}`];
      ...
      expect(page.locator(".govuk-notification-banner__content")).toContainText(
        confirm_content.feedbackBannerMessage,
      ),
      expect(page.locator(".govuk-notification-banner__content")).toContainText(
        confirm_content.feedbackBannerText,
      ),
