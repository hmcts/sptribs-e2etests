...
type CreateFlags4Page = {
    ...
    cancel: string;
    next: string;
    checkPageLoadsReasonableAdjustAltFormat(page:Page, accessibilityTest: boolean, caseNumber: string,):Promise<void>;
...
async checkPageLoadsReasonableAdjustAltFormat(page:Page, accessibilityTest: boolean, caseNumber: string,): Promise<void> {
    await page.waitForSelector(`#flag-type-heading:text-is("${createFlags4ReasonableAdjustment_content.subTitle1}")`);
    ...
    await Promise.all([
        ...
        expect(
           page.locator(`#flag-type-heading:text-is("${createFlags4ReasonableAdjustment_content.subTitle1}")`),
         ).toBeVisible(),

         ...Array.from({ length: 8 }, (_, index: number) => {
          const textOnPage: ArrayConstructor = (createFlags4ReasonableAdjustment_content as any)[
            `textOnPage${index + 1}`
          ];
...
async selectAdjustmentType(page, adjustmentType): Promise<void> {
        ...
        await page.waitForSelector(`.govuk-label:text-is(" Add comments for this flag")`)
},
