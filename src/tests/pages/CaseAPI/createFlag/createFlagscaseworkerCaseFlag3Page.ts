...
type CreateFlags3Page = {
  ...
  cancel: string;
  next: string;
  checkPageLoadsReasonableAdjustment(
    page: Page,
...
const createFlags3Page: CreateFlags3Page = {
  ...
  async checkPageLoadsReasonableAdjustment(
    page: Page,
    accessibilityTest: boolean,
    caseNumber: string,
  ): Promise<void> {
    await page.waitForSelector(
      `#flag-type-heading:text-is("${createFlags3ReasonableAdjustment_content.subTitle1}")`,
    );
    ...
    await Promise.all([
      ...
      ...Array.from({ length: 8 }, (_, index: number) => {
        const textOnPage: ArrayConstructor = (
          createFlags3ReasonableAdjustment_content as any
        )[`textOnPage${index + 1}`];
  ...
  async selectAdjustmentType(page, adjustmentType): Promise<void> {
    ...
    await page.waitForSelector(
      `h1#flag-type-heading:text-is("${adjustmentType}")`,
    );
  },
};
