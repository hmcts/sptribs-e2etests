...
import confirm_content from "../../../fixtures/content/CaseAPI/createFlag/confirm_content.ts";
...
const createFlagConfirmPage: ConfirmPage = {
  closeAndReturn: ".button",

  async checkPageLoads(page, accessibilityTest, caseNumber): Promise<void> {
    await page.waitForSelector(`h1:text-is("${confirm_content.subTitle1}")`);
    ...
    await Promise.all([
      ...
      commonHelpers.checkVisibleAndPresent(
        page.locator(`markdown > h2:text-is("${confirm_content.subTitle2}")`),
        1,
      ),
...
export default createFlagConfirmPage;
