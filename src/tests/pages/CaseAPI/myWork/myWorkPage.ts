...
type MyWorkPage = {
  ...
  selectAvailableTasks(page: Page, accessibilityTest: boolean): Promise<void>;
};
  ...
  async checkPageLoads(page: Page, accessibilityTest: boolean): Promise<void> {
    ...
    await Promise.all([
      ...
      ...Array.from({ length: 4 }, (_, index) => {
        ...
        return commonHelpers.checkVisibleAndPresent(
          page.locator(`li.hmcts-sub-navigation__item:text-is("${tab}")`),
          1,
        );
      }),
      ...Array.from({ length: 7 }, (_, index) => {
        ...
        return commonHelpers.checkVisibleAndPresent(
          page.locator(`.govuk-table__header:text-is("${column}")`),
          1,
        );
      }),
    ]);
