...
const caseFilterPage: CaseFilterPage = {
  ...
  async checkPageLoads(page: Page, accessibilityTest: boolean): Promise<void> {
    await expect(page.locator(".govuk-heading-xl")).toHaveText(caseFilter_content.pageTitle)
    await expect(page.locator(".form-label").nth(0)).toHaveText(caseFilter_content.textOnPage1)
    await expect(page.locator(".form-label").nth(1)).toHaveText(caseFilter_content.textOnPage2)
    await expect(page.locator(".form-label").nth(2)).toHaveText(caseFilter_content.textOnPage3)
